...
declare version=$1
declare crds=(application-crd applicationset-crd appproject-crd)
...
for crd in ${crds[@]};
  do declare crdpath=https://raw.githubusercontent.com/argoproj/argo-cd/$version//manifests/crds/$crd.yaml && \
    curl -vOL "$crdpath";
done
